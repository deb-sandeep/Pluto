 ...
 */
public class PSTable extends JTable implements DragGestureListener {

    /** Default serial version UID. */
    private static final long serialVersionUID = 1L ;

    /** Log4J logger for this class. */
    public static final Logger logger = Logger.getLogger( PSTable.class ) ;
    ...
    private final MouseListener mouseListener = new MouseAdapter() {
        ...
        public void mouseClicked( final MouseEvent e ) {
            ...
            int                     modelRowId  = -1 ;
            PSTableModel            model       = null ;
            StockTradeGrouping      stock       = null ;
            ScripITD                scrip       = null ;

            if( e.getClickCount() == 2 ) {
                final int row = PSTable.this.rowAtPoint( e.getPoint() ) ;
                if( row != -1 ) {
                    final String scripName = null ;
                    try {
                        // Convert the row Id to model specific row identifier.
                        modelRowId = convertRowIndexToModel( row ) ;
                        model = ( PSTableModel )getModel() ;
                        stock = model.getStockTradeGrouping( modelRowId ) ;
                        scrip = ScripITDValueCache.getInstance().
                                getScripITDForSymbol( stock.getSymbol() ) ;
                        ...
                        cfg = new ChartEntityConfig( stock.getSymbol(),
    ...
    public void dragGestureRecognized( final DragGestureEvent dge ) {
        ...
        StockTradeGrouping stock = null ;
        ScripITD           scrip = null ;
        final PSTableModel       model = ( PSTableModel )getModel() ;
        ...
        // Convert the row Id to model specific row identifier.
        modelRowId = convertRowIndexToModel( rowId ) ;
        stock      = model.getStockTradeGrouping( modelRowId ) ;
        symbol     = stock.getSymbol() ;
        scrip      = ScripITDValueCache.getInstance().getScripITDForSymbol( symbol ) ;

        final ChartEntityConfig cfg = new ChartEntityConfig(
                                   symbol, EntityType.SCRIP, scrip.getTime() ) ;
